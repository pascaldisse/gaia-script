 ...
 * 
 * Includes support for the Base64 number encoding system.
 */

// Import required modules
const fs = require('fs');
const path = require('path');
const base64Numbers = require('./base64_numbers');
...
function parseGaiaScript(code) {
  const ast = {
    ...
    layers: [],
    base64Numbers: [] // New array to store base64 number references
  };
  ...
  for (const line of lines) {
    ...
    // Network definition
    if (line.match(/N\s*〈/) || line.match(/N\s*⟨/)) {
      ast.type = 'Network';
      const components = line.match(/[〈⟨](.*?)[〉⟩]/);
      if (components) {
        ast.components = components[1].split(/[⊕,]/); // Support both ⊕ and comma separators
      }
    }
    ...
    // Scan the line for Base64 number patterns
    const base64Matches = line.matchAll(/#⟨([A-Za-z0-9+/]+)⟩/g);
    for (const match of base64Matches) {
      const base64Value = match[1];
      const decimalValue = base64Numbers.fromBase64Number(base64Value);
      
      ast.base64Numbers.push({
        base64: base64Value,
...
function compileToJS(ast, inputPath) {
  ...
  js += `// Base64 number system implementation\n`;
  js += `const BASE64_ALPHABET = "${base64Numbers.BASE64_ALPHABET}";\n\n`;
  
  js += `// Convert decimal to Base64\n`;
  js += `function toBase64Number(n) {\n`;
  js += `  if (n === 0) return "A";\n`;
  js += `  const digits = [];\n`;
  js += `  while (n > 0) {\n`;
  js += `    const remainder = n % 64;\n`;
  js += `    digits.push(BASE64_ALPHABET[remainder]);\n`;
  js += `    n = Math.floor(n / 64);\n`;
  js += `  }\n`;
  js += `  return digits.reverse().join('');\n`;
  js += `}\n\n`;
  
  js += `// Convert Base64 to decimal\n`;
  js += `function fromBase64Number(s) {\n`;
  js += `  let value = 0;\n`;
  js += `  for (let i = 0; i < s.length; i++) {\n`;
  js += `    const char = s[i];\n`;
  js += `    const digit = BASE64_ALPHABET.indexOf(char);\n`;
  js += `    if (digit === -1) throw new Error("Invalid base64 character: " + char);\n`;
  js += `    value = value * 64 + digit;\n`;
  js += `  }\n`;
  js += `  return value;\n`;
  js += `}\n\n`;
  
  js += `// Parse GaiaScript Base64 number format (#⟨...⟩)\n`;
  js += `function parseBase64Number(str) {\n`;
  js += `  const base64Match = str.match(/#⟨([A-Za-z0-9+/]+)⟩/);\n`;
  js += `  if (!base64Match) throw new Error("Invalid Base64 number format: " + str);\n`;
  js += `  return fromBase64Number(base64Match[1]);\n`;
  js += `}\n\n`;
  
  js += `// Format decimal as GaiaScript Base64 number\n`;
  js += `function formatBase64Number(num) {\n`;
  js += `  return "#⟨" + toBase64Number(num) + "⟩";\n`;
  js += `}\n\n`;
  
  js += `// Base64 number examples\n`;
  js += `const BASE64_EXAMPLES = {\n`;
  js += `  ZERO: "A",          // 0\n`;
  js += `  ONE: "B",           // 1\n`;
  js += `  TEN: "K",           // 10\n`;
  js += `  SIXTY_FOUR: "BA",   // 64\n`;
  js += `  HUNDRED: "Bc",      // 100\n`;
  js += `  THOUSAND: "Pd",     // 1000\n`;
  js += `  MILLION: "3+8w",    // 1000000\n`;
  js += `  BILLION: "4c9184"   // 1000000000\n`;
  js += `};\n\n`;
  
  // Base64 numbers found in the code
  if (ast.base64Numbers.length > 0) {
    js += `// Base64 numbers used in this GaiaScript code\n`;
    js += `const GAIA_BASE64_NUMBERS = {\n`;
    for (const num of ast.base64Numbers) {
      js += `  "${num.base64}": ${num.decimal}, // ${num.original}\n`;
    }
    js += `};\n\n`;
  }
  ...
  js += `  base64Numbers: ${ast.base64Numbers.length > 0 ? 'GAIA_BASE64_NUMBERS' : '{}'},\n`;
...
try {
  ...
  // Create HTML wrapper for web target
  if (htmlOutputPath) {
    // Use a custom name for the output HTML file if it's the main.gaia file
    if (path.basename(inputPath) === 'main.gaia') {
      htmlOutputPath = path.join(path.dirname(htmlOutputPath), 'gaia-hub.html');
    }
