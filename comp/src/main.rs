...
use gaiascript::compilers::macos_compiler::MacOSCompiler;
use gaiascript::ast::SymbolTable;

use gaiascript::platform_detector;
use gaiascript::compilers::windows_compiler::WindowsCompiler;
use gaiascript::compilers::linux_compiler::LinuxCompiler;
use gaiascript::compilers::android_compiler::AndroidCompiler;
...
fn main() {
    ...
    if args.len() < 2 {
        println!("Usage:");
        println!("  cargo run -- auto <filename>           # Auto-detect platform and compile");
        ...
        println!("    Platforms: js, kotlin, react, flutter, lynx, wasm, x86_64, arm64, llvm, macos");
        println!("  cargo run -- macos <filename> <app_name> [output_dir]  # Compile to macOS native app");
        println!("  cargo run -- windows <filename> [app_name] [output_dir]  # Compile to Windows native app");
        println!("  cargo run -- linux <filename> [app_name] [output_dir]  # Compile to Linux native app");
        println!("  cargo run -- ios <filename> <app_name> [output_dir]  # Compile to iOS native app");
        println!("  cargo run -- android <filename> <app_name> [output_dir]  # Compile to Android native app");
    ...
    match args[1].as_str() {
        "auto" => {
            if args.len() < 3 {
                println!("Error: Usage: auto <filename>");
                return;
            }
            
            let filename = &args[2];
            // Detect platform and compile accordingly
            let platform = gaiascript::platform_detector::determine_best_target();
            println!("Auto-detected platform: {:?}", platform);
            
            match platform {
                gaiascript::platform_detector::Platform::MacOS => {
                    let app_name = Path::new(filename).file_stem().unwrap().to_str().unwrap();
                    compile_to_macos(filename, app_name, ".");
                },
                gaiascript::platform_detector::Platform::Windows => {
                    compile_to_windows(filename);
                },
                gaiascript::platform_detector::Platform::Linux => {
                    compile_to_linux(filename);
                },
                gaiascript::platform_detector::Platform::IOS => {
                    let app_name = Path::new(filename).file_stem().unwrap().to_str().unwrap();
                    compile_to_ios(filename, app_name, ".");
                },
                gaiascript::platform_detector::Platform::Android => {
                    let app_name = Path::new(filename).file_stem().unwrap().to_str().unwrap();
                    compile_to_android(filename, app_name, ".");
                },
                _ => {
                    // Default to JavaScript for Web or unknown platforms
        ...
        "macos" => {
            if args.len() < 4 {
                println!("Error: Usage: macos <filename> <app_name> [output_dir]");
                return;
            }
            ...
            compile_to_macos(filename, app_name, output_dir);
        },
        "windows" => {
            if args.len() < 3 {
                println!("Error: Usage: windows <filename> [app_name] [output_dir]");
                return;
            }
            
            let filename = &args[2];
            let app_name = if args.len() >= 4 {
                &args[3]
            } else {
            ...
            compile_to_windows_with_options(filename, app_name, output_dir);
        },
        "linux" => {
            if args.len() < 3 {
                println!("Error: Usage: linux <filename> [app_name] [output_dir]");
                return;
            }
            ...
            compile_to_linux_with_options(filename, app_name, output_dir);
        },
        "ios" => {
            if args.len() < 4 {
                println!("Error: Usage: ios <filename> <app_name> [output_dir]");
                return;
            }
            ...
            compile_to_ios(filename, app_name, output_dir);
        },
        "android" => {
            if args.len() < 4 {
                println!("Error: Usage: android <filename> <app_name> [output_dir]");
                return;
            }
            ...
            compile_to_android(filename, app_name, output_dir);
        },
        "llvm" => {
            if args.len() < 3 {
    ...
    match parser::parse(&contents) {
        Ok(ast) => {
            let target_name = match target {
                ...
                gaiascript::asm_compiler::AsmTarget::WASMUI => "wasmui",
            };
            ...
            let asm_extension = match target {
                ...
                gaiascript::asm_compiler::AsmTarget::WASMUI => "wat",
            };
            ...
            match fs::write(&asm_filename, asm_code) {
                Ok(_) => {
                    ...
                    match target {
                        ...
                        gaiascript::asm_compiler::AsmTarget::WASMUI => {
                            println!("To convert to binary WebAssembly UI component:");
                            println!("wat2wasm {} -o {}_ui.wasm", asm_filename, stem);
                        },
                    }
                },
                Err(e) => {
    ...
    match parser::parse(&contents) {
        Ok(ast) => {
            ...
            let (output_code, extension) = match platform {
                ...
                "macos" => {
                    // Just add a placeholder here, we'll handle actual macOS compilation 
                    // in the specific macos command path
                    ("// macOS compilation requires the 'macos' command.".to_string(), "swift".to_string())
                },
                _ => {
                    println!("Unknown platform: {}", platform);
                    println!("Supported platforms: js, kotlin, react, flutter, lynx, wasm, x86_64, arm64, llvm, macos");
                    return;
                }
            };
...
/// Compile to Windows with the Windows compiler
fn compile_to_windows(filename: &str) {
    let contents = match fs::read_to_string(filename) {
        Ok(c) => c,
        Err(e) => {
            println!("Error reading file: {}", e);
    ...
    match parser::parse(&contents) {
        Ok(ast) => {
            println!("Compiling to Windows native application...");
            
            // Create a Windows compiler
            let mut compiler = WindowsCompiler::new();
            
            // Compile the AST to C# code
            match compiler.compile(&ast) {
                Ok(csharp_code) => {
                    compiler.output_code = csharp_code;
                    
                    // Generate app name from filename
                    let path = Path::new(filename);
                    let app_name = path.file_stem().unwrap().to_str().unwrap();
                    
                    // Generate the Visual Studio project
                    match compiler.generate_vs_project(app_name, ".") {
                        Ok(_) => {
                            println!("Generated Windows project at {}", app_name);
                            println!("To build the application:");
                            println!("  cd {}", app_name);
                            println!("  dotnet build");
                            println!("  dotnet run");
                        },
                        Err(e) => {
                            println!("Error generating Visual Studio project: {}", e);
                        }
                    }
                },
                Err(e) => {
...
/// Compile to Windows with options
fn compile_to_windows_with_options(filename: &str, app_name: &str, output_dir: &str) {
    let contents = match fs::read_to_string(filename) {
        Ok(c) => c,
        Err(e) => {
            println!("Error reading file: {}", e);
        ...
        Ok(ast) => {
            ...
            match compiler.compile(&ast) {
                Ok(csharp_code) => {
                    ...
                    // Generate the Visual Studio project
                    match compiler.generate_vs_project(app_name, output_dir) {
                        Ok(_) => {
                            println!("Generated Windows project at {}/{}", output_dir, app_name);
                            println!("To build the application:");
                            println!("  cd {}/{}", output_dir, app_name);
                            println!("  dotnet build");
                            println!("  dotnet run");
                        },
                        Err(e) => {
...
/// Compile to Linux with the Linux compiler
fn compile_to_linux(filename: &str) {
    let contents = match fs::read_to_string(filename) {
        Ok(c) => c,
        Err(e) => {
            println!("Error reading file: {}", e);
    ...
    match parser::parse(&contents) {
        Ok(ast) => {
            println!("Compiling to Linux native application...");
            
            // Create a Linux compiler
            let mut compiler = LinuxCompiler::new();
            
            // Compile the AST to C code
            match compiler.compile(&ast) {
                Ok(c_code) => {
                    compiler.output_code = c_code;
                    ...
                    match compiler.generate_build_files(app_name, ".") {
                        Ok(_) => {
                            println!("Generated Linux project at {}", app_name);
                            println!("To build the application:");
                            println!("  cd {}", app_name);
                            println!("  ./build.sh");
                            println!("  ./{}  # Run the application", app_name);
                        },
                        Err(e) => {
                            println!("Error generating build files: {}", e);
                        }
                    }
                },
                Err(e) => {
...
/// Compile to Linux with options
fn compile_to_linux_with_options(filename: &str, app_name: &str, output_dir: &str) {
    let contents = match fs::read_to_string(filename) {
        Ok(c) => c,
        Err(e) => {
            println!("Error reading file: {}", e);
        ...
        Ok(ast) => {
            ...
            match compiler.compile(&ast) {
                Ok(c_code) => {
                    ...
                    // Generate the build files
                    match compiler.generate_build_files(app_name, output_dir) {
                        Ok(_) => {
                            println!("Generated Linux project at {}/{}", output_dir, app_name);
                            println!("To build the application:");
                            println!("  cd {}/{}", output_dir, app_name);
                            println!("  ./build.sh");
                            println!("  ./{}  # Run the application", app_name);
...
/// Compile to iOS with the macOS compiler (since both use Swift)
fn compile_to_ios(filename: &str, app_name: &str, output_dir: &str) {
    let contents = match fs::read_to_string(filename) {
        Ok(c) => c,
        Err(e) => {
            println!("Error reading file: {}", e);
    ...
    match parser::parse(&contents) {
        Ok(ast) => {
            println!("Compiling to iOS native application...");
            
            // Use macOS compiler for iOS since they share the same language (Swift)
            let mut compiler = MacOSCompiler::new();
            
            // iOS-specific imports
            compiler.imports.push("import UIKit".to_string());
            
            // Compile the AST to Swift code
            match compiler.compile(&ast) {
                Ok(swift_code) => {
                    // Replace AppKit with UIKit
                    let ios_code = swift_code
                        .replace("import AppKit", "import UIKit")
                        .replace("import SwiftUI", "import SwiftUI\nimport UIKit")
                        .replace("NSApplication", "UIApplication")
                        .replace("NSWindow", "UIWindow")
                        .replace("NSView", "UIView");
                    
                    compiler.output_code = ios_code;
                    
                    // Generate the Xcode project for iOS
                    let ios_dir = format!("{}/{}_iOS", output_dir, app_name);
                    let ios_dir_path = Path::new(&ios_dir);
                    
                    if !ios_dir_path.exists() {
                        fs::create_dir_all(&ios_dir).unwrap_or_else(|_| {
                            println!("Error creating iOS project directory");
                        });
                    }
                    
                    // Write the iOS Swift code
                    let swift_path = ios_dir_path.join("main.swift");
                    match fs::write(&swift_path, &compiler.output_code) {
                        Ok(_) => {
                            println!("Generated iOS Swift code at {}/main.swift", ios_dir);
                            println!("Note: To create an iOS app, open this code in Xcode and configure an iOS app project");
                        },
                        Err(e) => {
                            println!("Error writing Swift file: {}", e);
                        }
                    }
                },
                Err(e) => {
...
/// Compile to Android with the Android compiler
fn compile_to_android(filename: &str, app_name: &str, output_dir: &str) {
    let contents = match fs::read_to_string(filename) {
        Ok(c) => c,
        Err(e) => {
            println!("Error reading file: {}", e);
    ...
    match parser::parse(&contents) {
        Ok(ast) => {
            println!("Compiling to Android application...");
            
            // Create an Android compiler
            let mut compiler = AndroidCompiler::new();
            
            // Compile the AST to Kotlin code
            match compiler.compile(&ast) {
                Ok(kotlin_code) => {
                    compiler.output_code = kotlin_code;
                    
                    // Generate the Android project
                    match compiler.generate_android_project(app_name, output_dir) {
                        Ok(_) => {
                            println!("Generated Android project at {}/{}", output_dir, app_name);
                            println!("To build the application:");
                            println!("  cd {}/{}", output_dir, app_name);
                            println!("  ./gradlew build");
                            println!("  Open the project in Android Studio to run on device or emulator");
                        },
                        Err(e) => {
                            println!("Error generating Android project: {}", e);
                        }
                    }
                },
                Err(e) => {
...
/// Compile GaiaScript to a macOS native application
fn compile_to_macos(filename: &str, app_name: &str, output_dir: &str) {
    let contents = match fs::read_to_string(filename) {
        Ok(c) => c,
        Err(e) => {
            println!("Error reading file: {}", e);
    ...
    match parser::parse(&contents) {
        Ok(ast) => {
            println!("Compiling to native macOS application...");
            
            // Create a macOS compiler
            let mut compiler = MacOSCompiler::new();
            ...
            match compiler.compile(&ast) {
                Ok(swift_code) => {
                    ...
                    compiler.output_code = swift_code;
                    
                    // Generate the Xcode project
                    match compiler.generate_xcode_project(app_name, output_dir) {
                        Ok(_) => {
                            println!("Generated macOS project at {}/{}", output_dir, app_name);
                            println!("To build the application:");
                            println!("  cd {}/{}", output_dir, app_name);
                            println!("  ./build.sh");
                            
                            // Ask if user wants to build now
                            println!("Do you want to build the application now? (y/n)");
                            let mut input = String::new();
                            std::io::stdin().read_line(&mut input).unwrap_or(0);
                            
                            if input.trim().to_lowercase() == "y" || input.trim().to_lowercase() == "yes" {
                                // Build the app
                                match compiler.build_app(app_name, output_dir) {
                                    Ok(_) => {
                                        println!("Successfully built {}/{}/{}", output_dir, app_name, app_name);
                                        println!("Run the application with:");
                                        println!("  open {}/{}/{}", output_dir, app_name, app_name);
                                    },
                                    Err(e) => {
                                        println!("Error building application: {}", e);
                                    }
                                }
                            }
                        },
                        Err(e) => {
                            println!("Error generating Xcode project: {}", e);
                        }
                    }
                },
                Err(e) => {
